...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        connectionCount++;
        if ((connectionCount > 4 && connectionCount <= 8) ||
            (connectionCount > 8 && connectionCount % 2 == 0))
        {
            send(client_socket, MIRROR_TOKEN, strlen(MIRROR_TOKEN), 0);
            close(client_socket);
        }
        else
        {
            ...
            else if (pid == 0)
            {
                ...
                send(client_socket, SERVER_TOKEN, strlen(SERVER_TOKEN), 0);
...
void create_tarball_dGetFiles(char *output_filename, char *search_path, time_t date1, time_t date2)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        else if (S_ISREG(statbuf.st_mode))
        {
            ...
            // printf("dates: %s %s %s %d %d\n", ctime(&file_date), ctime(&date1), ctime(&date2), difftime(file_date, date1), difftime(file_date, date2));
            ...
            if (difDays1 >= 0 && difDays2 <= 0)
            {
                ...
                // Add the file to the tarball
                // printf("found file %s. adding to tar\n", current_path);
                snprintf(tar_command, MAX_PATH_LENGTH + 50, "tar -rf '%s' '%s' > /dev/null 2>/dev/null", output_filename, current_path);
                system(tar_command);
